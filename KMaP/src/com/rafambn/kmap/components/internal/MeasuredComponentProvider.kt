...
internal class MeasuredComponentProvider(
    ...
    private fun getAndMeasure(index: Int, constraints: Constraints): MeasuredComponent =
        MeasuredComponent(
            index = index,
            placeables = measureScope.compose(index).map { it.measure(constraints) },
    ...
    fun getAndMeasureMarker(markerIndex: Int): MeasuredComponent {
        ...
        return getAndMeasure(actualIndex, childConstraints)
    }
    ...
    fun getAndMeasureCanvas(canvasIndex: Int, constraints: Constraints): MeasuredComponent {
        val actualIndex =
            componentProvider.markersCount * 2 + componentProvider.pathsCount + canvasIndex // Canvas are at [markersCount*2+pathsCount ... end]
        return getAndMeasure(actualIndex, constraints)
    }
}
